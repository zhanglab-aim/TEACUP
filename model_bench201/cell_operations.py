...
__all__ = ['NAS_BENCH_201', 'NAS_BENCH_201_1D', 'NAS_BENCH_201_3D', 'OPS', 'ResNetBasicblock', 'SearchSpaceNames']

OPS = {
    'none': lambda C_in, C_out, stride, affine, track_running_stats: Zero(C_in, C_out, stride),
    'avg_pool_3x3x3': lambda C_in, C_out, stride, affine, track_running_stats: POOLING(C_in, C_out, stride, 'avg', affine, track_running_stats, 3),
    ...
    'avg_pool_3': lambda C_in, C_out, stride, affine, track_running_stats: POOLING(C_in, C_out, stride, 'avg', affine, track_running_stats, 1),
    ...
    'nor_conv_3x3x3': lambda C_in, C_out, stride, affine, track_running_stats: ReLUConvBN(C_in, C_out, (3, 3, 3), (stride, stride, stride), (1, 1, 1), (1, 1, 1), affine, 3, track_running_stats),
    'nor_conv_3x3': lambda C_in, C_out, stride, affine, track_running_stats: ReLUConvBN(C_in, C_out, (3, 3), (stride, stride), (1, 1), (1, 1), affine, 2, track_running_stats),
    'nor_conv_3': lambda C_in, C_out, stride, affine, track_running_stats: ReLUConvBN(C_in, C_out, 3, stride, 1, 1, affine, 1, track_running_stats),
    'nor_conv_1x1x1': lambda C_in, C_out, stride, affine, track_running_stats: ReLUConvBN(C_in, C_out, (1, 1, 1), (stride, stride, stride), (0, 0, 0), (1, 1, 1), affine, 3, track_running_stats),
    'nor_conv_1x1': lambda C_in, C_out, stride, affine, track_running_stats: ReLUConvBN(C_in, C_out, (1, 1), (stride, stride), (0, 0), (1, 1), affine, 2, track_running_stats),
    'nor_conv_1': lambda C_in, C_out, stride, affine, track_running_stats: ReLUConvBN(C_in, C_out, 1, stride, 0, 1, affine, 1, track_running_stats),
...
NAS_BENCH_201_3D = ['none', 'skip_connect',
                 'nor_conv_1x1x1', 'nor_conv_3x3x3', 'avg_pool_3x3x3']
NAS_BENCH_201 = ['none', 'skip_connect',
                 'nor_conv_1x1', 'nor_conv_3x3', 'avg_pool_3x3']
NAS_BENCH_201_1D = ['none', 'skip_connect',
                 'nor_conv_1', 'nor_conv_3', 'avg_pool_3']
DARTS_SPACE = ['none', 'skip_connect', 'dua_sepc_3x3', 'dua_sepc_5x5',
               'dil_sepc_3x3', 'dil_sepc_5x5', 'avg_pool_3x3', 'max_pool_3x3']

SearchSpaceNames = {'connect-nas': CONNECT_NAS_BENCHMARK,
                    'nas-bench-201': NAS_BENCH_201,
                    'nas-bench-201-1D': NAS_BENCH_201_1D,
                    'nas-bench-201-3D': NAS_BENCH_201_3D,
                    'darts': DARTS_SPACE}
...
class ReLUConvBN(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, padding, dilation, affine, dim=2, track_running_stats=True):
        super(ReLUConvBN, self).__init__()
        if dim == 3:
            self.op = nn.Sequential(
                nn.ReLU(inplace=False),
                nn.Conv3d(C_in, C_out, kernel_size, stride=stride,
...
class DualSepConv(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, padding, dilation, affine, track_running_stats=True):
        ...
        self.op_a = SepConv(C_in, C_in, kernel_size, stride,
                            padding, dilation, affine, track_running_stats)
        self.op_b = SepConv(C_in, C_out, kernel_size, 1,
                            padding, dilation, affine, track_running_stats)
...
class ResNetBasicblock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride, affine=True, dim=2):
        super(ResNetBasicblock, self).__init__()
        assert stride == 1 or stride == 2, 'invalid stride {:}'.format(stride)
        assert dim == 1 or dim == 2 or dim == 3, 'invalid dim {:}'.format(dim)
        self.conv_a = ReLUConvBN(inplanes, planes, 3, stride, 1, 1, affine, dim)
        _init(self.conv_a, mode=MODE)
        self.conv_b = ReLUConvBN(planes, planes, 3, 1, 1, 1, affine, dim)
        _init(self.conv_b, mode=MODE)
        if stride == 2:
            if dim == 3:
                self.downsample = nn.Sequential(
                    nn.AvgPool3d(kernel_size=2, stride=2, padding=0),
                    nn.Conv3d(inplanes, planes, kernel_size=1, stride=1, padding=0, bias=False))               
            elif dim == 2:
                self.downsample = nn.Sequential(
            ...
            self.downsample = ReLUConvBN(inplanes, planes, 1, 1, 0, 1, affine, dim)
        else:
            self.downsample = None
        if self.downsample: _init(self.downsample, mode=MODE)
...
class POOLING(nn.Module):
    ...
    def __init__(self, C_in, C_out, stride, mode, affine=True, track_running_stats=True, dim=2):
        super(POOLING, self).__init__()
        if C_in == C_out:
            self.preprocess = None
        else:
            self.preprocess = ReLUConvBN(C_in, C_out, 1, 1, 0, 1, affine, dim, track_running_stats)
        if mode == 'avg':
            if dim == 3:
            ...
            if dim == 3:
                self.op = nn.MaxPool3d(3, stride=stride, padding=1)
            elif dim == 2:
                self.op = nn.MaxPool2d(3, stride=stride, padding=1)
            elif dim == 1:
                self.op = nn.MaxPool1d(3, stride=stride, padding=1)
        else:

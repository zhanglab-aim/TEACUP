...
from amber.architect import GeneralController

from measure_training_free_reward import get_profile, get_reward
from utils import get_data_info, get_fake_model_space_rl, get_tmp_data
...
def main():
    ...
    model_space = get_fake_model_space_rl()
    ...
    # Build controller
    controller = GeneralController(model_space=model_space, rescale_advantage_by_reward=True, train_pi_iter=1, temperature=2.)


    # Prepare log file
    timestamp = "{:}".format(time.strftime("%Y%m%d-%H%M%S"))
    job_name = f"{args.dataset}/RL_iter{args.max_iter}x{args.buffer_size}" + \
    ...
    for idx in range(args.max_iter):
        try:
            controller, df, te_mean_all, params_all = rl_iter(args, te_metric, te_weight, controller, data_info, df, 
...
def gen_results(idx, controller):
    ...
    d = {}
    for i, v in enumerate(controller.get_action()[1]):
        d[str(i)] = list([float(_) for _ in v[0]])
...
def rl_iter(args, te_metric, te_weight, controller, data_info, df, tmp_data, episode, save_dir, te_mean_all, params_all):
    count = 0
    te_mean_episode = []
    action_episode = []
    while count < args.sample_size:
        ...
        arc_str = ''.join([str(layer) for layer in arc])
        ...
        with session_scope():
            ...
            final_reward, all_rewards = get_reward(tmp_data, data_info, model=model, 
                                                te_metrics=te_metric, weights=te_weight, 
                                                verbose=args.verbose)
            if final_reward == -100.:
                continue

            te_mean_all.append(final_reward)
        ...
        te_mean_episode.append(final_reward) # TODO will only use for ranking purpose
        action_episode.append(arc)

        if args.verbose: print(f"Generated arch: {arc_str}, got {te_metric} = {final_reward} | {params}")

        # Store results to controller
        if not args.ranking_reward:
            controller.store(action=arc, prob=prob, reward=final_reward)
        count += 1

        # Store results to dataframe
        df.loc[len(df)] = [episode, time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), arc_str, flops / 10**9 / 2, params / 10**6, final_reward] + all_rewards
    ...
    if args.ranking_reward:
        ...
        for _rank, _arc in zip(te_mean_episode_ranks, action_episode):
            controller.store(action=_arc, prob=prob, reward=_rank)
    #######################

    # Update the model space distribution
    if not args.warm_up or episode+2 > args.buffer_size:
        if len(controller.buffer.action_buffer) > 0:
            controller.buffer.finish_path(state_space=controller.model_space, global_ep=episode, working_dir=save_dir)
        controller.train()
